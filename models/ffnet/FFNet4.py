...
from ..layers import RegressionModel, ClassificationModel, AnchorPoints, Segmentation_Head, build_deformable_detr, build_position_encoding, MLP
...
# FFNet2_4，在FFNet2_1基础上修改
class FFNet4(nn.Module):
    def __init__(self):
        super().__init__()
        out_channels = [96, 192, 384] # 256 # 16 + 32 + 64
        hidden_dim = 256
        ...
        self.deformable_detr = build_deformable_detr(out_channels, num_feature_levels=2)
        ...
        # self.regression = RegressionModel(num_features_in=hidden_dim, num_anchor_points_list=num_anchor_points)
        # self.class_embed = ClassificationModel(num_features_in=hidden_dim, \
        #                                           num_classes=self.num_classes, \
        #                                           num_anchor_points_list=num_anchor_points)
        self.regression = MLP(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=2, num_layers = 3)
        self.class_embed = MLP(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=2, num_layers = 3)


        self.anchor_points1 = AnchorPoints(pyramid_levels=[2, ], row=row, line=line)
        self.anchor_points2 = AnchorPoints(pyramid_levels=[3, ], row=row, line=line)
    ...
    def forward(self, samples: NestedTensor):
        ...
        # rectangle deformable detr
        nested_features_list = []
        features_pos_list = []
        for l, fea in enumerate([pool1, pool2]):
            features_mask = F.interpolate(samples.mask.float().unsqueeze(1), size=fea.shape[-2:]).squeeze(1).bool()
            nested_features = NestedTensor(fea, features_mask)
            nested_features_list.append(nested_features)

            features_pos = self.position_embedding(nested_features).to(fea.dtype)
            features_pos_list.append(features_pos)
        hs = self.deformable_detr(samples, nested_features_list, features_pos_list)


        batch_size = x.shape[0]
        # run the regression and classification branch
        regression = self.regression(hs) * 10  # 8x
        classification = self.class_embed(hs)
        anchor_points1 = self.anchor_points1(x).repeat(batch_size, 1, 1)
        anchor_points2 = self.anchor_points2(x).repeat(batch_size, 1, 1)
        anchor_points = torch.cat([anchor_points1, anchor_points2], dim=1)
...
def build_ffnet4(args, training):
