...
class FFNet4_2(nn.Module):
    def __init__(self):
        super().__init__()
        out_channels = [16, 32] # 256 # 16 + 32 + 64
        hidden_dim = 256
        ...
        self.deformable_detr = build_deformable_detr(out_channels, num_feature_levels=len(out_channels))
        ...
        self.regression = MLP(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=2 * num_anchor_points, num_layers = 3)
        self.class_embed = MLP(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=2 * num_anchor_points, num_layers = 3)
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs_split = hs.split([fea.tensors.shape[-2] * fea.tensors.shape[-1] for fea in nested_features_list], dim=1)
        hs = hs_split[-1]

        batch_size = x.shape[0]
        # run the regression and classification branch
        regression = self.regression(hs).reshape(batch_size, -1, 2) * 10  # 8x
        classification = self.class_embed(hs).reshape(batch_size, -1, 2)
...
def build_ffnet4_2(args, training):
