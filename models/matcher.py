...
class HungarianMatcher_Crowd_Points(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    ...
    def forward(self, outputs, targets):
        ...
        Returns:
            ...
            For each batch element, it holds:
                len(index_i) = len(index_j) = min(num_queries, num_target_points)
        """
        bs, num_queries = outputs["anchor_points"].shape[:2]

        # We flatten to compute the cost matrices in a batch
        out_points = outputs["anchor_points"].flatten(0, 1)  # [batch_size * num_queries, 2]
...
def build_matcher_crowd(args):
    ...
    elif args.match_type == 'anchor_points':
        return HungarianMatcher_Crowd_Points()
    raise ValueError(f"args.match_type={args.match_type } is not found!")
